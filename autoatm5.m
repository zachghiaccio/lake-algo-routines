...
indyplot = 0; % Display plots = 1, no plots = 0
multiplot = 1;
...
for k = 1:length(fnames(1:9)) % Loops through the given 9 ILATM1b files
    fname = fnames{k};
    ...
    for i = 1:bin_count
        ...
        if any(n > lake_thresh)
            ...
            %-------------------------------------------------------------------------%
            %% Lake Surface/Bed Identification - See atm_chunking_sub.m and
            ...
            sigma_btm = movstd(window_lake_btm,100,'omitnan'); % Bottom uncertainty
            
            %% Polynomial Fitting
            if lake_start == 1 % If lake is at beginning of data file
                idx = isnan(window_lake_btm);
                [p,S,mu] = polyfit(dist(~idx), window_lake_btm(~idx), 3);
                [lake_btm_fitted,delta] = polyval(p, dist, S, mu);
            elseif any(length(elev_bin)-10:length(elev_bin) == lake_end) % If lake is at end of data file
                ...
                resid = window_lake_btm-lake_btm_fitted;
                delta = sqrt(nansum(resid(~idx).^2)/length(window_lake_btm(~idx)));
            end
                ...
                if ~isnan(lake_depth_polyfit)
                    ...
                    if multiplot    
                        ...
                        if lake_count~=10 && lake_count~=13 && lake_count~=15
                            ...
                            ylim([min(elev_bin)-2 max(elev_bin)])
                            grid on;
                            title([num2str(lat_bin(marked_int)), char(0176), 'N, ', num2str(lon_bin(marked_int)), char(0176), 'E'], 'FontWeight', 'bold')
                            keyboard;
                        end
